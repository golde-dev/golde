...
const userNameSchema = z
  .string()
  .min(1, { message: "IAM user name must be at least 1 character long" })
  .max(64, { message: "IAM User name must be at most 64 characters long" })
  .regex(/^[A-Za-z0-9+=,.@-]+$/, {
    message: "IAM user name can only contain alphanumeric characters and +=,.@-",
  });
